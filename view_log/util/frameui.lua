...
function file_open(name)
    -- 获取一个打开文件名
    name = name or QCommonDlg.getOpenFileName("打开Lua脚本文件", "", "Lua脚本(*.lua);;全部文件(*)")
    -- 以只读方式打开文件
    file = io.open(name,"r")
    if file then
...
menuBar = mainWindow:menuBar(){
	...
	QMenu("工具(&T)"){
        QAction("YModem"){
            triggered = function() mdiArea:addSubWindow( YModemView()  ):show() end, 
			QKeySequence("Ctrl+Y"),
        },
	},
...
editMenu:addAction(QAction("清除日志(&C)"){ triggered = function() logEdit:clear() end })
